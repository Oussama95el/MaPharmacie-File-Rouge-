...
export default {
  ...
  methods: {
    ...
    removeCategory(id) {
      axios
        .delete(
          `http://localhost/MaPharmacie/backend/public/category/delete/${id}`
        )
        .then((res) => {
    ...
    addCategory(nom) {
      const endpoint =
        "http://localhost/MaPharmacie/backend/public/category/add";
      axios.post(endpoint, { nom }).then((res) => {
        if (res.status !== 200) {
          this.message = "Api connection failed";
        } else {
          this.categories = this.categories.filter(
            (e) => e.id != this.categories.id
          );
        }
      });
    },
    startUpdate(newData) {
      const endpoint =
        "http://localhost/MaPharmacie/backend/public/category/update";
